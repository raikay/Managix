...
using Managix.Redis.ServerIteration;
...
namespace Managix.Redis.Implementations
{
    ...
    public partial class RedisDatabase : IRedisDatabase
    {
        ...
        private readonly ServerEnumerationStrategy _serverEnumerationStrategy = new();
        private readonly string _keyPrefix;
        private readonly uint _maxValueLength;
        ...
        public RedisDatabase(
                IRedisConnectionPoolManager connectionPoolManager, IJsonSerializer serializer, ServerEnumerationStrategy serverEnumerationStrategy,
                int dbNumber, uint maxvalueLength, string keyPrefix, ILoggerFactory loggerFactory)
        {
            Serializer = serializer ?? throw new ArgumentNullException(nameof(serializer));
            _serverEnumerationStrategy = serverEnumerationStrategy ?? new ServerEnumerationStrategy();
            _connectionPoolManager = connectionPoolManager ?? throw new ArgumentNullException(nameof(connectionPoolManager));
            _dbNumber = dbNumber;
            _keyPrefix = keyPrefix;
            _maxValueLength = maxvalueLength;
            Logger = loggerFactory.CreateLogger(nameof(RedisDatabase));
        }
        ...
        public IDatabase Database
        {
            get
            {
                var db = _connectionPoolManager.GetConnection().GetDatabase(_dbNumber);
        ...
        {
            var entryBytes = value.OfValueSize(Serializer, _maxValueLength, key);
        ...
        {
            var values = items
                .OfValueInListSize(Serializer, _maxValueLength)
        ...
        {
            pattern = $"{_keyPrefix}{pattern}";
            var keys = new HashSet<string>();

            var multiplexer = Database.Multiplexer;
            var servers = ServerIteratorFactory.GetServers(_connectionPoolManager.GetConnection(), _serverEnumerationStrategy).ToArray();
            ...
            return !string.IsNullOrEmpty(_keyPrefix)
                        ? keys.Select(k => k.Substring(_keyPrefix.Length))
                        : keys;
        }
